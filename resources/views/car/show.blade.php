...
@section('content')
            ...
            <div>
                <div class="p-6 lg:p-10">
                    <a href="/" class="inline-block text-black my-3">
                    ...
                    <div class="flex flex-col items-center justify-center text-center">
                        ...
                        @else
                            <a href="/reservations/cars/{{ $car->id }}" class="block bg-denim1 text-white mt-6 py-2 px-3 rounded-xl hover:opacity-80"><i class="fa-solid fa-calendar-days"></i>
                ...
                @auth
                    @if ($car->uid == auth()->user()->id)
                        <div class="mt-4 p-2 flex justify-center gap-6">
                            ...
                            <form class="inline" method="POST" action="/cars/{{ $car->id }}">
                                @csrf
                                @method('DELETE')
                ...
                <div class="bg-gray-50 border border-gray-200 p-4 lg:p-10 rounded lg:mx-52 mt-4">
                    ...
                    @auth
                        @if ($car->uid != auth()->user()->id && !($current_user_review->count()))
                            ...
                            <form class="rating-submit" method="POST" action="/reviews/cars/{{ $car->id }}">
                                @csrf
                        ...
                        @endif
                    @else
                            <p><a href="/login" class="text-slate-600">Sign in</a> to post a review</p>
                    @endauth
                </div>
            </div>
